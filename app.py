...
Environment variables:
    ...
    PERIOD_PORT               Fallback HTTP port (default: 8080)
    PERIOD_HOST               Listen host (default: 0.0.0.0)
...
async def unlock(req: Request):
    ...
    resp.set_cookie(
        ...
        httponly=True,
        secure=(req.headers.get("x-forwarded-proto", req.url.scheme).split(",")[0].strip().lower() == "https"),
        samesite="strict",
        max_age=60 * 60 * 24 * 90,
    )
    return resp
...
<body><div class="wrap">
  ...
  <div class="footer">period-tracker · Zeabur MCP edition</div>
</div>
<script>
...
app.mount("/", period_mcp.streamable_http_app())
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        app,
        host=os.environ.get("PERIOD_HOST", "0.0.0.0"),
        port=int(os.environ.get("PORT", os.environ.get("PERIOD_PORT", "8080"))),
    )
